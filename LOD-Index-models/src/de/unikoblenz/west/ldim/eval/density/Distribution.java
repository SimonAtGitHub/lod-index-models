...
public abstract class Distribution {
	...
	/**
	 * Reads the frequency distribution from an input file (zip compressed). Builds the map of event frequencies,  
	 * 
	 * @param fin Input file to read the frequencies from. See SortedCountAggregator for details on format.
	 * @throws IOException
	 */
	public void readFreqFile(File fin) throws IOException {
		...
		while ((line = zIn.getBufferedReader().readLine()) != null) {
			// entries are line based. each line contains the String representation of the event, followed by a separator, followed by the number of observations
	...
	/**
	 * Set the value for an observation.
	 * Adjusts the frequencies data of observations and the count of count, i.e. frequency distribution
	 * 
	 * @param key String representation of the event for which to update the number of observation.
	 * @param freq New number of observations.
	 */
	public void setCount(String key, int freq) {
		if (this.frequencies.containsKey(key)) {
			...
			// adjust count of counts, i.e. remove the count of the old frequency by one
	 ...
	 *  
	 * @param x String representation of the event for which the frequency count
	 *            is to be returned
