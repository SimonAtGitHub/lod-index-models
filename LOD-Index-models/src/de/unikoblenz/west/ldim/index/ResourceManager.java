 ...
 * as well as dependencies between these files. The advantage is that this
 * allows for an efficient re-use of partial results. When asked for the file of
 * a specific index, the ResourceManager simply returns a reference to the file
 * if it exists already. If the file does not exists, the according index is
 * computed and requests the needed input files (which in turn are re-used if
 * the exist already). In this sense the ResourceManager has been inspired by
 * the idea of Makefiles -- but a hard coded version.
 * 
 * The access to specific files is managed via constant values. The filenames
 * are internally computed and are composed on the basis of the initial input
 * file. The ResourceManager keeps track of all generated files and is capable
 * to tidy up, i.e. to delete all files generated by it.
 * 
...
public class ResourceManager {
	...
	/**
	 * Constant for denoting the file related to a list of nquads sorted by
	 * subject
	 */
	...
	/**
	 * Constant for denoting the file related to a type set (TS) Index sorted by
	 * the key entry
	 */
	...
	/**
	 * Constant for denoting the file related to a property set (PS) Index
	 * sorted by the key entry
	 */
	public static final byte INDEX_K_PS = 6;
	/**
	 * Constant for denoting the file related to a property set (PS) Index
	 * sorted by the value entry
	 */
	public static final byte INDEX_V_PS = 7;
	/**
	 * Constant for denoting the file related to an extended characteristic set
	 * (ECS) Index sorted by the key entry
	 */
	public static final byte INDEX_K_ECS = 8;
	/**
	 * Constant for denoting the file related to an extended characteristic set
	 * (ECS) Index sorted by the value entry
	 */
	public static final byte INDEX_V_ECS = 9;
	/**
	 * Constant for denoting the file related to an incoming property set (IPS)
	 * Index sorted by the key entry
	 */
	public static final byte INDEX_K_IPS = 10;
	/**
	 * Constant for denoting the file related to an incoming property set (IPS)
	 * Index sorted by the value entry
	 */
	public static final byte INDEX_V_IPS = 11;
	/**
	 * Constant for denoting the file related to a SchemEX Index sorted by the
	 * key entry
	 */
	...
	/**
	 * Constant for denoting the file related to a subject value index sorted by
	 * the key entry
	 */
	...
	/**
	 * Constant for denoting the file related to a predicate value index sorted
	 * by the key entry
	 */
	...
	/**
	 * Constant for denoting the file related to a object value index sorted by
	 * the key entry
	 */
	...
	/**
	 * Constant for denoting the file related to a context value index sorted by
	 * the key entry
	 */
	...
	/**
	 * Constant for denoting the file related to a pay level domain (PLD) index
	 * sorted by the key entry
	 */
	...
	/**
	 * Constant for denoting the file related to a rdf:type index index sorted
	 * by the key entry
	 */
	...
	/**
	 * Constant for denoting the file related to a keyword based index sorted by
	 * the key entry
	 */
	...
	/**
	 * Constant for denoting the file related to a SchemEX Index generated using
	 * a JOIN sorted by the key entry
	 */
	public static final byte INDEX_K_SCHEMEX_JOIN = 28;
	/**
	 * Constant for denoting the file related to a SchemEX Index generated using
	 * a JOIN sorted by the value entry
	 */
	public static final byte INDEX_V_SCHEMEX_JOIN = 29;
	/**
	 * Constant for denoting the file related to a ECS index referring to
	 * contexts (instead of USUs) sorted by the key entry
	 */
	public static final byte INDEX_K_ECS_CONTEXT = 30;
	/**
	 * Constant for denoting the file related to a ECS index referring to
	 * contexts (instead of USUs) sorted by the value entry
	 */
	public static final byte INDEX_V_ECS_CONTEXT = 31;

	/**
	 * Internal constant used for serializing the type of file together with the
	 * input file.
	 */
	private static final String SEPARATOR = "___";
	/**
	 * Internal flag whether files should be re-generated each time they are
	 * requested. This flag was mainly used during development and is now set to
	 * false by default.
	 */
	...
	/**
	 * Returns a reference to a file containing data of a certain type. The type
	 * of requested data is given by a constant. If the file exists, the method
	 * simply returns the according File object, pre-set with all correct and
	 * relevant values. If the file does not exist it is created on the basis of
	 * required input files. For retrieving input files, the method is called
	 * internally in a re-cursive fashion.
	 * 
	 * @param type
	 *            Type of data file asked for (see constants for possible
	 *            values)
	 * @param raw
	 *            Original base input file from which the requested data file
	 *            should be generated.
		...
		if (available.containsKey(key)) {
			...
			if ((!outFile.exists()) || ResourceManager.OVERRIDE) {
				...
				case ResourceManager.SORTED_SUBJECT: {
					NQuadSorting subjectSort = new NQuadSorting(
							NQuadSorting.SUBJECT_SORT);
				...
				case ResourceManager.SORTED_PREDICATE: {
					NQuadSorting predicateSort = new NQuadSorting(
							NQuadSorting.PREDICATE_SORT);
					long t1 = System.currentTimeMillis();
				...
				case ResourceManager.SORTED_OBJECT: {
					NQuadSorting objectSort = new NQuadSorting(
							NQuadSorting.OBJECT_SORT);
					long t1 = System.currentTimeMillis();
				...
				case ResourceManager.SORTED_CONTEXT: {
					NQuadSorting contextSort = new NQuadSorting(
							NQuadSorting.CONTEXT_SORT);
					long t1 = System.currentTimeMillis();
				...
				case ResourceManager.INDEX_K_SCHEMEX: {
					...
					this.sortIndex(schemexIndexFile, outFile,
							IndexSorting.KEY_SORT);
					long t2 = System.currentTimeMillis();
					System.out.println(((t2 - t1) / 1000) + " sec");
				}
					break;
				...
				case ResourceManager.INDEX_K_SCHEMEX_JOIN: {
					File schemexJIndexFile = this
							.get(INDEX_V_SCHEMEX_JOIN, raw);
					long t1 = System.currentTimeMillis();
					System.out
							.print("Sorting SchemEX (Join) index by key entry... ");
					this.sortIndex(schemexJIndexFile, outFile,
							IndexSorting.KEY_SORT);
					long t2 = System.currentTimeMillis();
					System.out.println(((t2 - t1) / 1000) + " sec");
				}
				...
				case ResourceManager.INDEX_K_P_HASH: {
					...
					long t1 = System.currentTimeMillis();
					System.out
							.print("Sorting Predicate index by key entry... ");
				...
				case ResourceManager.INDEX_V_PLD_HASH: {
					PldHashIndex pldIndex = new PldHashIndex(new File(
				...
				case ResourceManager.INDEX_K_TYPE: {
					...
					this.sortIndex(typeIndexFile, outFile,
							IndexSorting.KEY_SORT);
					long t2 = System.currentTimeMillis();
					System.out.println(((t2 - t1) / 1000) + " sec");
				}
				...
				case ResourceManager.INDEX_K_KEYWORD: {
					...
					this.sortIndex(keywordIndexFile, outFile,
							IndexSorting.KEY_SORT);
					long t2 = System.currentTimeMillis();
					System.out.println(((t2 - t1) / 1000) + " sec");
				}
				...
				case ResourceManager.INDEX_K_ECS_CONTEXT: {
					File ecsContextIndexFile = this.get(INDEX_V_ECS_CONTEXT,
							raw);
					long t1 = System.currentTimeMillis();
					System.out
							.print("Sorting ECS (context lookup) index by key entry... ");
					this.sortIndex(ecsContextIndexFile, outFile,
							IndexSorting.KEY_SORT);
					long t2 = System.currentTimeMillis();
					System.out.println(((t2 - t1) / 1000) + " sec");
				}
	...
	/**
	 * Compose a String key for internal use of maintaining a list to all
	 * references of already existing files.
	 * 
	 * @param type
	 *            Type of file
	 * @param raw
	 *            Original file for which to come up with a composed key.
	 * @return Key value for internal use to store references to file.
	 */
	...
	/**
	 * Generic method for sorting index files according to different modes (See
	 * IndexSorting for details)
	 * 
	 * @param in
	 *            input index file
	 * @param out
	 *            sorted output index file
	 * @param mode
	 *            sorting mode.
	 */
	...
	/**
	 * Proposes a (hopefully) unique name for a temporary file. The temporary
	 * names are composed of the name of base file (raw), the current time and a
	 * random number.
	 * 
	 * @param raw
	 *            Original base file for which to generate a temporary file
	 * @return Filename proposal for a temporary file.
	 */
	private File composeTmpFile(File raw) {
		File result = null;
		String fname = raw.getName() + "-" + System.currentTimeMillis() + "-"
	...
	/**
	 * Generates the effective filenames for derived files (indices or sorted
	 * files). The file name depends on a base file and some meaningful,
	 * explanatory suffix.
	 * 
	 * @param type Type of file requested
	 * @param raw Original base file for which the filename is requested
	 * @return File object representing the data file storing the requested information
	 */
	private File composeFile(byte type, File raw) {
		File result = null;
		String fname = null;
		switch (type) {
		case ResourceManager.SORTED_SUBJECT:
	...
	/**
	 * Deletes all index files and intermediate files. Aggregated files
	 * (frequency counts and inverted indices) are not managed by the
	 * ResourceManager and are not deleted.
	 */
